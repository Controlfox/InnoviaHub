...
import {
  PersonalitySettings,
  ReceptionistPersonalityComponent,
} from '../../components/receptionist-personality/receptionist-personality.component';
import {
  AdminTab,
  AdminTabId,
  NavTabsComponent,
} from '../../components/nav-tabs/nav-tabs.component';
import { ReceptionistProfileService } from '../../services/receptionist-profile.service';
...
@Component({
  ...
  //lägg till NavTabsComponent i imports
  imports: [
    CommonModule,
    FormsModule,
    ReceptionistPersonalityComponent,
...
export class ProfilePageComponent implements OnInit {
  //Tabs-konfiguration
  tabs: AdminTab[] = [
    { id: 'bookings' as AdminTabId, label: 'Alla bokningar' },
    { id: 'sensors' as AdminTabId, label: 'Alla sensorer' },
    { id: 'receptionist' as AdminTabId, label: 'Receptionisten' },
  ];
  activeId: AdminTabId = 'bookings' as AdminTabId;

  // enkel handler från NavTabs
  ...
  profile: PersonalitySettings | null = null;

  ngOnInit() {
    const userId = this.authService.getUserId();
    this.receptionistProfileService.getProfile(userId).subscribe({
      next: (res: any) => {
        this.profile = {
          assistantName: res.assistantName ?? '',
          tone: res.tone ?? 'neutral',
          style: res.style ?? 'balanced',
          emoji: Number.isFinite(res.emoji) ? res.emoji : 0,
        };
      },
      error: (err) => console.error('Kunde inte hämta profil', err),
    });
    // Load user data once
    this.loadUserData();
    // Load bookings
  ...
  private async loadUserBookings() {
    ...
    try {
      const apiUrl =
        (window as any).__env?.NG_APP_API_URL || 'http://localhost:5184';
      const fullUrl = `${apiUrl}/api/booking/user/${userId}`;
      console.log('🌐 Making API call to:', fullUrl);

      this.bookings =
        (await this.http.get<BookingDto[]>(fullUrl).toPromise()) || [];
  ...
  async saveEdit() {
    ...
    try {
      ...
      const endDateOnly = new Date(
        originalEndTime.getFullYear(),
        originalEndTime.getMonth(),
        originalEndTime.getDate()
      );
      ...
      await this.bookingService
        .update(this.selectedBooking.id, updateDto)
        .toPromise();
  ...
  async deleteBooking() {
    ...
    if (
      !confirm(
        `Är du säker på att du vill radera bokningen för ${this.selectedBooking.resourceName}?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await this.bookingService
        .delete(this.selectedBooking.id, userId)
        .toPromise();
  ...
  onSavePersonality(settings: PersonalitySettings) {
    const userId = this.authService.getUserId();
    this.receptionistProfileService.saveProfile(userId, settings).subscribe({
      next: (res) => console.log('Profil sparad', res),
      error: (err) => console.error('Kunde inte spara profil', err),
    });
  }
}
